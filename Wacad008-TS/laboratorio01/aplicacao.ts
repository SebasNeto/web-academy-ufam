type TuplaLembrete = [number, string, Date, Date | undefined, string | undefined];

let db: TuplaLembrete[] = [];
let countId = 1;

const tituloTxt = document.querySelector('#tituloTxt') as HTMLInputElement;
const dataFim = document.querySelector('#dataFim') as HTMLInputElement;
const descTxt = document.querySelector('#descTxt') as HTMLTextAreaElement;
const lembreteId = document.querySelector('#lembreteId') as HTMLInputElement;
...
const divLista = document.querySelector('#lista') as HTMLDivElement;
...
const atualizarTela = () => {
    ...
    db.forEach(item => {
        ...
        if (prazo) html += `<small><b>Prazo:</b> ${prazo.toLocaleString()}</small><br>`;
        if (texto) html += `<p>${texto}</p>`;
...
const carregarParaEdicao = (id: number) => {
